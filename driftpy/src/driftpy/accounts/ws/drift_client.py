...
class WebsocketDriftClientAccountSubscriber(DriftClientAccountSubscriber):
    ...
    async def subscribe_to_oracle(self, full_oracle_wrapper: FullOracleWrapper):
        ...
        async with self.oracle_subscriber._lock:
            if full_oracle_wrapper.pubkey in self.oracle_subscriber.pubkey_to_subscription:
                ...
                # 建立 oracle_id 到 subscription_id 的映射
                subscription_id = self.oracle_subscriber.pubkey_to_subscription[full_oracle_wrapper.pubkey]
                self.oracle_subscriber.oracle_id_to_subscription[oracle_id] = subscription_id
                # 存储解码器和初始数据
                self.oracle_subscriber.decode_map[oracle_id] = get_oracle_decode_fn(full_oracle_wrapper.oracle_source)
                if full_oracle_wrapper.oracle_price_data_and_slot is not None:
                    self.oracle_subscriber.data_map[oracle_id] = full_oracle_wrapper.oracle_price_data_and_slot
                return
    ...
    async def subscribe_to_oracle_info(
        ...
        async with self.oracle_subscriber._lock:
            if oracle_info.pubkey in self.oracle_subscriber.pubkey_to_subscription:
                ...
                if oracle_info.pubkey not in self.oracle_subscriber.pubkey_to_oracle_ids:
                    self.oracle_subscriber.pubkey_to_oracle_ids[oracle_info.pubkey] = set()
                self.oracle_subscriber.pubkey_to_oracle_ids[oracle_info.pubkey].add(oracle_id)
                # 建立 oracle_id 到 subscription_id 的映射
                subscription_id = self.oracle_subscriber.pubkey_to_subscription[oracle_info.pubkey]
                self.oracle_subscriber.oracle_id_to_subscription[oracle_id] = subscription_id
                # 存储解码器
                self.oracle_subscriber.decode_map[oracle_id] = get_oracle_decode_fn(source)
                return
    ...
    async def _set_perp_oracle_map(self):
        ...
        for market in perp_markets:
            ...
            # ⭐ 修复：检查 oracle_id 而不是 pubkey（因为 data_map 的 key 现在是 str）
