...
def get_auction_price(order: Order, slot: int, oracle_price: int) -> int:
    slot = max(slot +1,order.slot)
    # 先处理 TriggerMarket 且带 OracleTriggerMarket 位 ——> oracle-offset
    if is_variant(order.order_type, "TriggerMarket") and (
        order.bit_flags & OrderBitFlag.OracleTriggerMarket
    ) != 0:
        return get_auction_price_for_oracle_offset_auction(order, slot, oracle_price)

    # 其余 Market / TriggerMarket / TriggerLimit ——> fixed
    ...
    # Limit：有 oracle_price_offset ——> oracle-offset，否则 fixed
    if is_variant(order.order_type, "Limit"):
        if getattr(order, "oracle_price_offset", 0) != 0:
            return get_auction_price_for_oracle_offset_auction(order, slot, oracle_price)
        else:
            return get_auction_price_for_fixed_auction(order, slot)

    # Oracle ——> oracle-offset
...
def get_auction_price_for_fixed_auction(order: Order, slot: int) -> int:
    ...
    if is_variant(order.direction, "Long"):
        # 修正这一行：改为标准插值
        price_delta = (order.auction_end_price - order.auction_start_price) * delta_numerator // delta_denominator
    else:
        price_delta = (order.auction_start_price - order.auction_end_price) * delta_numerator // delta_denominator
